...
// Séparation du fichier en lignes et en colonnes
  ...
  await saveFile(FICHIER, JSON.stringify(jsonGamePlay))

  const allPlayers = Array.from({ length: 99 }, (v, i) => (i+1).toString());

  function getArrayUnique(players: any[]) {
    return Array.from(new Set(players))
      .sort((a, b) => {return +a - +b});
  }

  function getPlayerType(playerTypeList: String[]) {
    let players = []
    playerTypeList.forEach((playerType: string) => {
...
// away
  ...
  await saveFile(FICHIER_2, `window.chargerDonneesLineup(${JSON.stringify(lineUp)})`)
}
